...
class PgVectorStore:  # pragma: no cover - needs a live Postgres
    ...
    def upsert(self, chunks: Sequence[Chunk], vectors: Sequence[Sequence[float]]) -> int:
        ...
        with self.conn.cursor() as cur:
            cur.executemany(
                ...
                "heading_path=EXCLUDED.heading_path", rows)
        return len(rows)
    ...
    def keyword_search(self, query: str, k: int = 10, where: dict[str, Any] | None = None) -> list[ScoredChunk]:
        clause, meta_params = _where(where)
    ...
    def all_chunks(self) -> list[Chunk]:
        rows = self.conn.execute(
            f"SELECT chunk_id, doc_id, text, source, ordinal, heading_path, metadata FROM {self.table}"
        ).fetchall()
...
def _where(where: dict[str, Any] | None) -> tuple[str, list[Any]]:
    parts, params = [], []
